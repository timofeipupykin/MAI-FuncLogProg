...
average_grade_per_subject :-
    ...
    print_avg_for_subjects(Subjects).

print_avg_for_subjects([]).
print_avg_for_subjects([Subject|Rest]) :-
    findall(Mark, grade(_, _, Subject, Mark), Marks),
    sum_list(Marks, Sum),
    length(Marks, Count),
    Avg is Sum / Count,
    format('~w: ~2f~n', [Subject, Avg]),
    print_avg_for_subjects(Rest).

% --- Количество не сдавших студентов по группам ---
failed_students_per_group :-
    setof(Group, S^Subj^M^grade(Group, S, Subj, M), Groups),
    print_failed_per_group(Groups).

print_failed_per_group([]).
print_failed_per_group([Group|Rest]) :-
    setof(Student, Subj^M^(grade(Group, Student, Subj, M), M < 3), FailedStudents),
    length(FailedStudents, Count),
    format('Группа ~w: ~w не сдавших~n', [Group, Count]),
    print_failed_per_group(Rest).

% --- Количество не сдавших студентов по каждому предмету ---
failed_students_per_subject :-
    setof(Subject, G^S^M^grade(G, S, Subject, M), Subjects),
    print_failed_per_subject(Subjects).

print_failed_per_subject([]).
print_failed_per_subject([Subject|Rest]) :-
    setof(Student, G^M^(grade(G, Student, Subject, M), M < 3), FailedStudents),
    length(FailedStudents, Count),
    format('~w: ~w не сдавших~n', [Subject, Count]),
    print_failed_per_subject(Rest).